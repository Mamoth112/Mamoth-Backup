...
db.prepare(`
  CREATE TABLE IF NOT EXISTS settings (
    ...
    leveling_channel_id TEXT DEFAULT NULL,
    ...
    support_type_ban INTEGER DEFAULT 0 NOT NULL,
    support_type_billing INTEGER DEFAULT 0 NOT NULL,
    support_type_bug INTEGER DEFAULT 0 NOT NULL,
    support_type_question INTEGER DEFAULT 0 NOT NULL,
    support_type_request INTEGER DEFAULT 0 NOT NULL,
    support_type_suggest INTEGER DEFAULT 0 NOT NULL,
    support_type_support INTEGER DEFAULT 0 NOT NULL,
    support_type_ticket INTEGER DEFAULT 0 NOT NULL,
    support_Id_ban INTEGER DEFAULT 0 NOT NULL,
    support_Id_billing INTEGER DEFAULT 0 NOT NULL,
    support_Id_bug INTEGER DEFAULT 0 NOT NULL,
    support_Id_question INTEGER DEFAULT 0 NOT NULL,
    support_Id_request INTEGER DEFAULT 0 NOT NULL,
    support_Id_suggest INTEGER DEFAULT 0 NOT NULL,
    support_Id_support INTEGER DEFAULT 0 NOT NULL,
    support_Id_ticket INTEGER DEFAULT 0 NOT NULL
  );
`).run();
...
const settings = {
  ...
  selectTicketChannelPanelId: db.prepare('SELECT ticket_panel_channel_id FROM settings WHERE guild_id = ?;'),
  ...
  selectLevelingChannelId: db.prepare('SELECT leveling_channel_id FROM settings WHERE guild_id = ?;'),
  ...
  selectCrown: db.prepare(`
    ...
    WHERE guild_id = ?;
  `),
  listSupportTypes: db.prepare(`
  SELECT support_type_ban, support_type_billing, support_type_bug, support_type_question, support_type_request, support_type_suggest, support_type_support, support_type_ticket
  FROM settings
  WHERE guild_id = ?;`),
  listSupportIds: db.prepare(`
  SELECT support_Id_ban, support_Id_billing, support_Id_bug, support_Id_question, support_Id_request, support_Id_suggest, support_Id_support, support_Id_ticket
  FROM settings
  WHERE guild_id = ?;`),
  ...
  updateTicketPanelChannelId: db.prepare('UPDATE settings SET ticket_panel_channel_id = ? WHERE guild_id = ?;'),
  updateModChannelIds: db.prepare('UPDATE settings SET mod_channel_ids = ? WHERE guild_id = ?;'),
  updateLevelingChannelId: db.prepare('UPDATE settings SET leveling_channel_id = ? WHERE guild_id = ?;'),
  ...
  updateSupportTypeBan: db.prepare('UPDATE settings SET support_type_ban = ? WHERE guild_id = ?;'),
  updateSupportTypeBilling: db.prepare('UPDATE settings SET support_type_billing = ? WHERE guild_id = ?;'),
  updateSupportTypeBug: db.prepare('UPDATE settings SET support_type_bug = ? WHERE guild_id = ?;'),
  updateSupportTypeQuestion: db.prepare('UPDATE settings SET support_type_question = ? WHERE guild_id = ?;'),
  updateSupportTypeRequest: db.prepare('UPDATE settings SET support_type_request = ? WHERE guild_id = ?;'),
  updateSupportTypeSuggest: db.prepare('UPDATE settings SET support_type_suggest = ? WHERE guild_id = ?;'),
  updateSupportTypeSupport: db.prepare('UPDATE settings SET support_type_support = ? WHERE guild_id = ?;'),
  updateSupportTypeTicket: db.prepare('UPDATE settings SET support_type_ticket = ? WHERE guild_id = ?;'),
  updateSupportIdBan: db.prepare('UPDATE settings SET support_Id_ban = ? WHERE guild_id = ?;'),
  updateSupportIdBilling: db.prepare('UPDATE settings SET support_Id_billing = ? WHERE guild_id = ?;'),
  updateSupportIdBug: db.prepare('UPDATE settings SET support_Id_bug = ? WHERE guild_id = ?;'),
  updateSupportIdQuestion: db.prepare('UPDATE settings SET support_Id_question = ? WHERE guild_id = ?;'),
  updateSupportIdRequest: db.prepare('UPDATE settings SET support_Id_request = ? WHERE guild_id = ?;'),
  updateSupportIdSuggest: db.prepare('UPDATE settings SET support_Id_suggest = ? WHERE guild_id = ?;'),
  updateSupportIdSupport: db.prepare('UPDATE settings SET support_Id_support = ? WHERE guild_id = ?;'),
  updateSupportIdTicket: db.prepare('UPDATE settings SET support_Id_ticket = ? WHERE guild_id = ?;'),
